@Int_CreateDispute
Feature: InternalPortal_DisputeMgt_CreateDispute
...
Scenario Outline: Create Dispute using valid IPS transaction ID
    ...
    Examples: 
      | Browser_Name| Portal_Name |    Participant_Name    |        user_id          |         password        |         Transaction_ID              |Disputer_Party| Disputed_Party|Dispute_Category  |Dispute_Type     |Dispute_Details  |
#     |    Chrome   | Internal    |     AlBilad Bank       |   dev_allAccess_user2   |   dev_allAccess_user2   | 20190808000000045C1B000000000003010 |AlBilad Bank  | Bank AlJazira |Customer complaint|Wrong information|Dispute Occoured |
      |    Chrome   | Internal    |     AlBilad Bank       |   dev_allAccess_user1   |   dev_allAccess_user1            | 20190808000000045C1B000000000003010 |AlBilad Bank  | Bank AlJazira |Customer complaint|Wrong information|Dispute Occoured |   
    # |    Firefox  | External    |     AlBilad Bank       |dev_SAALBI_allAccess_user|dev_SAALBI_allAccess_user| 20190808000000045C1B000000000003010 |AlBilad Bank  | Bank AlJazira |Customer complaint|Wrong information|Dispute Occoured |
###############################################################################################################################################################################################################
#####################################################################################################################################
@Int_CreateDispute_NonExistTransID
  Scenario Outline: Create Dispute using IPS transaction ID that does not exist 
    ...
    And Close the browser
    Examples: 
      | Browser_Name| Portal_Name |        user_id          |         password        |Participant_Name  | Non_Existing_Transaction_ID         | 
      |    Chrome   | Internal    |  dev_allAccess_user2    |   dev_allAccess_user2   |AlBilad Bank      | 20190808000000045C1B000000000003015 | 
#############################################################################################################################
#############################################################################################################################
...
Scenario Outline: Create Dispute using only 35 Numeric Transaction ID
    ...
    Examples: 
      | Browser_Name| Portal_Name |        user_id          |         password        |Participant_Name |   Non_Existing_Transaction_ID       | 
      |    Chrome   | Internal    |dev_allAccess_user2      |dev_allAccess_user2      | AlBilad Bank    |20190808000000045111000000000003010 | 
#############################################################################################################################
#############################################################################################################################
...
Scenario Outline: Cancel the Creation of Dispute using valid IPS transaction ID
 ...
 #   Then Click the "Close" button on "Confirmation of dispute creation" pop up screen
    And Close the browser
    Examples: 
      | Browser_Name| Portal_Name |        user_id          |         password        | Participant_Name|        Transaction_ID              |Disputer_Party| Disputed_Party|Dispute_Category  |Dispute_Type     |Dispute_Details  |
      |    Chrome   | Internal    |   dev_allAccess_user2   |dev_allAccess_user2      |  AlBilad Bank   |20190808000000045C1B000000000003010 |AlBilad Bank  | Bank AlJazira |Customer complaint|Wrong information|Dispute Occoured |
...
Scenario Outline: User can not Cancel the Creation of Dispute using valid IPS transaction ID
 ...
 #   Then Click the "Close" button on "Confirmation of dispute creation" pop up screen
    ...
    Examples: 
      | Browser_Name| Portal_Name |        user_id          |         password  |Participant_Name |         Transaction_ID              |Disputer_Party| Disputed_Party|Dispute_Category  |Dispute_Type     |Dispute_Details  |
      |    Chrome   | Internal    |  dev_allAccess_user2    |dev_allAccess_user2| AlBilad Bank    |20190808000000045C1B000000000003010 |AlBilad Bank  | Bank AlJazira |Customer complaint|Wrong information|Dispute Occoured |
    # |    Firefox  | External    |  dev_allAccess_user2    |dev_allAccess_user2| 20190808000000045C1B000000000003010 |AlBilad Bank  | Bank AlJazira |Customer complaint|Wrong information|Dispute Occoured |
###############################################################################################################################################################################################################
#####################################################################################################################################
...
Scenario Outline: Create Dispute using only 35 characters Transaction ID 
    ...
    Examples: 
      | Browser_Name| Portal_Name |        user_id          |         password        |Participant_Name  |   Non_Existing_Transaction_ID       | 
      |    Chrome   | Internal    |dev_allAccess_user2      |dev_allAccess_user2      |AlBilad Bank      |  ABCDDCBADFGTCFDPLKHBMNDGMNOPCBAMNAO | 
###############################################################################################
###############################################################################################
...
Scenario Outline: Create Dispute without entring IPS transaction ID
    ...
    And Close the browser
    Examples: 
      | Browser_Name| Portal_Name |        user_id          |         password        |Participant_Name | Blank_Transaction_ID | 
      |    Chrome   | Internal    |dev_allAccess_user2      |    dev_allAccess_user2  | AlBilad Bank    |                      | 
###############################################################################################
###############################################################################################
@Int_CreateDispute_TransIDlessthan35Alphanumeric
Scenario Outline: Create Dispute with Transaction ID less than 35 Characters
    Given Open browser "<Browser_Name>"
    When Login to "<Portal_Name>" Portal under "%ENV%" environment with username as "<user_id>" and password as "<password>"
    And Select "<Participant_Name>" from Participant drop down list
    Then Navigate to below mentioned menu in order to reach "Create Dispute" page
    ...
    And Close the browser
    Examples: 
       | Browser_Name| Portal_Name |        user_id          |         password        |Participant_Name  | Transaction_ID_less_than_35_Characters| 
       |    Chrome   | Internal    |   dev_allAccess_user2   |dev_allAccess_user2      |AlBilad Bank      | 20190808000000045C1B00                | 
 ##################################################################################
 ##################################################################################
 @Int_CreateDispute_TransIDmorethan35Alphanumeric
 Scenario Outline: Create Dispute with Transaction ID more than 35 Alphanumeric Characters
    Given Open browser "<Browser_Name>"
    When Login to "<Portal_Name>" Portal under "%ENV%" environment with username as "<user_id>" and password as "<password>"
    And Select "<Participant_Name>" from Participant drop down list
    Then Navigate to below mentioned menu in order to reach "Create Dispute" page
    ...
    And Close the browser
    Examples: 
       | Browser_Name| Portal_Name |        user_id          |         password        | Participant_Name |Transaction_ID_more_than_35_Characters| 
       |    Chrome   | Internal    |dev_allAccess_user2      |dev_allAccess_user2      | AlBilad Bank     |20190808000000045C1B00000000000301030 |
######################################################################################